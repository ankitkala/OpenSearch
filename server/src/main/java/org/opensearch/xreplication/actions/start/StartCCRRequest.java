...
public class StartCCRRequest extends ActionRequest implements ToXContent {
    public static String FOLLOWER_ALIASES = "follower_aliases";

    private final String[] followerAliases;

    public StartCCRRequest(RestRequest restRequest) {
        this(Strings.splitStringByCommaToArray(restRequest.param(FOLLOWER_ALIASES)));
    }
    public StartCCRRequest(String[] followers) {
        this.followerAliases = followers;
    }

    public StartCCRRequest(StreamInput in) throws IOException {
        super(in);
