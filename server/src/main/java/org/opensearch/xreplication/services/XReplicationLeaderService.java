...
import org.opensearch.xreplication.actions.syncsegments.SyncFromLeaderRequest;
...
public class XReplicationLeaderService implements IndexEventListener {
    ...
    private void initializeFollowerClients() {
        logger.info("remoteService.getRegisteredRemoteClusterNames(): {}", remoteService.getRegisteredRemoteClusterNames());

        Map<String, Client> newClients = remoteService.getRegisteredRemoteClusterNames().stream()
            .filter(alias -> !followerClients.containsKey(alias))
            .collect(Collectors.toMap(a -> a, a-> remoteService.getRemoteClusterClient(threadPool, a)));
        logger.info("new clients are {}", newClients);
        followerClients.putAll(newClients);
        /*
    ...
    public void syncFollowerSegments(IndexShard indexShard, ReplicationCheckpoint checkpoint) {
        initializeFollowerClients();
        List<ActionFuture<SyncFromLeaderResponse>> responseFutures = new ArrayList<>();
        logger.info("Notifying followers to sync segments");
        for (Map.Entry<String,Client> follower : followerClients.entrySet()) {
            //SyncFromLeaderRequest request = createSyncRequest(follower.getValue(), indexShard);
            logger.info("Notifying follower {}", follower.getKey());
            // invoke replication.
            responseFutures.add(follower.getValue().execute(SyncFromLeaderAction.INSTANCE, createSyncRequest(follower.getValue(), indexShard)));
            ///WORKING HERE.
        }
    ...
    private SyncFromLeaderRequest createSyncRequest(Client followerClient, IndexShard indexShard) {
        String indexName = indexShard.indexSettings().getIndex().getName();
        int shardID = indexShard.shardId().getId();
        ClusterState followerClusterState = getFollowerClusterState(followerClient, indexName, true, true);
        String followerIndexUUID = followerClusterState.getMetadata().index(indexName).getIndexUUID();
        ShardId followerShardID = new ShardId(indexName, followerIndexUUID, shardID);
        logger.info("ankikala routing info: {}", followerClusterState.getRoutingNodes());
        String[] nodes = followerClusterState.getRoutingNodes().shards(sr -> sr.shardId().getId() == shardID).stream().map(sr -> sr.currentNodeId()).toArray(String[] ::new);
        logger.info("nodes: {} {}", nodes[0], nodes[1]);
        return new SyncFromLeaderRequest(followerShardID, nodes);
    }

    private ClusterState getFollowerClusterState(Client followerClient, String indexName, Boolean includeNodes, Boolean includeRoutingTable) {
        ClusterStateRequest clusterStateRequest = followerClient.admin().cluster().prepareState()
            .clear()
            ...
            .request();
        return followerClient.admin().cluster().state(clusterStateRequest).actionGet().getState();
    }
